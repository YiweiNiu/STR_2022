...
# nohup Rscript analysis/aSTR.identification.permuted.R > analysis/aSTR.identification.permuted.log &
...
# permutate pudi (shuffle sample labels)
set.seed(1234)
rownames(rna_pudi) <- sample(rownames(rna_pudi), length(rownames(rna_pudi)))
...
# Save
saveRDS(fit_res, file = here::here("output", DOCNAME, "aSTR.pudi_str.glm_fit.permuted.rds"))
